...
- Run the display test program (test_{display}.py) or the DAL implementation
file (dal_{display}.py) to make sure the
...
communicate with the display. Currently, the low level SSD1306 driver does not
...
a single GPIO pin to send data to the 2D LED panel. Since all the tested microprocessors use
3.3V logic levels, a level shifter is necessary to correctly drive the data signal. With 
minor modifications, this driver should work with any compatible 2D display. The 
microprocessors may not support the 5V current drain necessary to drive your specific 
display panel correctly. 
...
----------------------
Development and tests were performed using the following versions of MicroPython;<p>
...
The Raspberry Pi Zero 2W files require sometimes extensive installation of
both system hardware and system drivers. Please consult the separate documentation
