...
# GKR 14.10.25
# Moved color functions(s) and constants to tftcolor.py
# GKR 29.06.25
# Added char_bkg(aColor) to define char background color
# Added clear() to clear screen
# Modified char to use bkgColor
# Modified fill to set bkgColor
# Modified rotation to use rotate offset
# Made size() a property
# Added optional drive parameter for ESP32 support
...
class ST7735(object) :
  ...
  def color( aR, aG, aB ) :
    ...
    return TFT.color565(aR, aG, aB)

  def __init__( self, spi, aDC, aReset, aCS, drive=-1) :
    ...
    if gl.platform == 'esp32s3' and drive != -1:
        self.dc  = Pin(aDC, Pin.OUT, Pin.PULL_DOWN, drive=drive)
        self.reset = Pin(aReset, Pin.OUT, Pin.PULL_DOWN, drive=drive)
        self.cs = Pin(aCS, Pin.OUT, Pin.PULL_DOWN, drive=Pin.drive)
    else:
...
#   @micropython.native
  def on( self, aTF = True ) :
    ...
    self._writecommand(self.DISPON if aTF else self.DISPOFF)

#   @micropython.native
  def invertcolor( self, aBool ) :
    '''Invert the color data IE: Black = White.'''
    self._writecommand(self.INVON if aBool else self.INVOFF)
...
#   @micropython.native
  def rotation( self, aRot ) :
    ...
    if (0 <= aRot < 4):
      ...
      self.rotate = aRot
      #If switching from vertical to horizontal swap x,y and offset
...
#   @micropython.native
  def text( self, aPos, aString, aColor, aFont, aSize = 1, nowrap = False ) :
    ...
    if (type(aSize) is int) or (type(aSize) is float):
      wh = (aSize, aSize)
    else:
      wh = aSize
  ...
  def char_bkg( self, aColor ):
    '''Set background color used to display text, see below.'''
    self.bkgColor = aColor
    ...
    if (startchar <= ci <= endchar):
      ...
      if aSizes[0] <= 1 and aSizes[1] <= 1 :
        ...
        for c in charA :
          ...
          for r in range(fonth) :
            if c & 0x01 :
              self.local_fill_rect((px, py), aSizes, aColor)
            else:
              self.local_fill_rect((px, py), aSizes, bColor)
            py += aSizes[1]
            c >>= 1
          px += aSizes[0]
...
#   @micropython.native
  # avoid child inheritance problems
  def local_fill_rect( self, aStart, aSize, aColor ) :
...
#   @micropython.native
  ...
  def fill( self, aColor = TFT.BLACK ) :
    '''Fill screen with the given color.'''
    self.local_fill_rect((0, 0), self._size, aColor)
  ...
  def setvscroll(self, tfa, bfa) :
    ...
    self._writecommand(self.VSCRDEF)
  ...
  def _vscrolladdr(self, addr) :
    self._writecommand(self.VSCSAD)
...
#   @micropython.native
  def _setwindowpoint( self, aPos ) :
    ...
    self._writecommand(self.CASET)            #Column address set.
...
#   @micropython.native
  ...
  def _setMADCTL( self ) :
    ...
    self._writecommand(self.MADCTL)
  ...
  def initb( self ) :
    ...
    self._reset()
    self._writecommand(self.SWRESET)              #Software reset.
    time.sleep_us(50)
    self._writecommand(self.SLPOUT)               #out of sleep mode.
    time.sleep_us(500)

    data1 = bytearray(1)
    self._writecommand(self.COLMOD)               #Set color mode.
    data1[0] = 0x05                             #16 bit color.
    self._writedata(data1)
    time.sleep_us(10)

    data3 = bytearray([0x00, 0x06, 0x03])       #fastest refresh, 6 lines front, 3 lines back.
    self._writecommand(self.FRMCTR1)              #Frame rate control.
    ...
    data2 = bytearray(2)
    self._writecommand(self.DISSET5)              #Display settings
    data2[0] = 0x15                             #1 clock cycle nonoverlap, 2 cycle gate rise, 3 cycle oscil, equalize
    data2[1] = 0x02                             #fix on VTL
    self._writedata(data2)

    self._writecommand(self.INVCTR)               #Display inversion control
    data1[0] = 0x00                             #Line inversion.
    self._writedata(data1)

    self._writecommand(self.PWCTR1)               #Power control
    data2[0] = 0x02   #GVDD = 4.7V
    data2[1] = 0x70   #1.0uA
    self._writedata(data2)
    time.sleep_us(10)

    self._writecommand(self.PWCTR2)               #Power control
    data1[0] = 0x05                             #VGH = 14.7V, VGL = -7.35V
    self._writedata(data1)

    self._writecommand(self.PWCTR3)           #Power control
    data2[0] = 0x01   #Opamp current small
    data2[1] = 0x02   #Boost frequency
    self._writedata(data2)

    self._writecommand(self.VMCTR1)               #Power control
    data2[0] = 0x3C   #VCOMH = 4V
    data2[1] = 0x38   #VCOML = -1.1V
    self._writedata(data2)
    time.sleep_us(10)

    self._writecommand(self.PWCTR6)               #Power control
    data2[0] = 0x11
    data2[1] = 0x15
    self._writedata(data2)
...
#                             0x1b, 0x23, 0x37, 0x00, 0x07, 0x02, 0x10])
    ...
    self._writecommand(self.GMCTRP1)
...
#                             0x30, 0x39, 0x3f, 0x00, 0x07, 0x03, 0x10])
    ...
    self._writecommand(self.GMCTRN1)
    ...
    self._writecommand(self.NORON)                #Normal display on.
    time.sleep_us(10)

    self._writecommand(self.RAMWR)
    time.sleep_us(500)

    self._writecommand(self.DISPON)
    self.cs(1)
    time.sleep_us(500)
  ...
  def initr( self ) :
    ...
    self._writecommand(self.FRMCTR2)              #Frame rate control.
    self._writedata(data3)

    data6 = bytearray([0x01, 0x2c, 0x2d, 0x01, 0x2c, 0x2d])
    self._writecommand(self.FRMCTR3)              #Frame rate control.
    ...
    self._writecommand(self.PWCTR4)               #Power control
    data2[0] = 0x8A   #Opamp current small
    data2[1] = 0x2A   #Boost frequency
    self._writedata(data2)

    self._writecommand(self.PWCTR5)               #Power control
    data2[0] = 0x8A   #Opamp current small
    ...
    self._writecommand(self.INVOFF)

    self._writecommand(self.MADCTL)               #Power control
    data1[0] = 0xC8
    self._writedata(data1)

    self._writecommand(self.COLMOD)
    data1[0] = 0x05
    self._writedata(data1)
